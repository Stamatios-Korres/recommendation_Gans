...
class BaseModule(nn.Module):
    ...
    def forward(self, users, items):
        ...
        preds = preds + self.item_biases(items)
        preds = preds +  (self.dropout(ues) * self.dropout(uis)).sum(1)
...
class BasePipeline:
    ...
    def __init__(self,
                 ...
                 loss_function=nn.MSELoss(reduction=False),
                 n_epochs=10,
                 verbose=False,
                 random_seed=None,
    ...
    def _fit_epoch(self, epoch=1, queue=None):
        ...
        for batch_idx, ((row, col), val) in pbar:
            ...
            batch_loss = loss.data.item() / row.size()[0]
